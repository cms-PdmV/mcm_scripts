...
requests = mcm.get('requests', query='member_of_campaign=%s&status=new' % (dest_chained_campaign))
...
for request in requests:
    ...
    root_id = chained_request['chain'][1]
    root_dr_req = mcm.get('requests', root_id)    

    if(root_id_req['keep_output'][0] != True and root_dr_req['status'] != 'done'):
        ...
        print('    https://cms-pdmv.cern.ch/mcm/requests?prepid='+str(root_id_req['prepid']))
